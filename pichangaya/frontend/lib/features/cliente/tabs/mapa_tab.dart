...
import 'package:pichangaya/features/cliente/providers/locales_provider.dart';
import 'package:pichangaya/shared/models/local_model.dart';

class MapaTab extends ConsumerStatefulWidget {
  final Function(LocalModel?)? onLocalSeleccionado;
  const MapaTab({super.key, this.onLocalSeleccionado});
  @override
  ConsumerState<MapaTab> createState() => _MapaTabState();
}

class _MapaTabState extends ConsumerState<MapaTab> {
  ...
  void _cargarLocales() {
    ...
    ref.read(localesProvider.notifier).cargar(
  ...
  void _mostrarPopup(LocalModel local) {
    ...
    _mapController.move(LatLng(local.lat - 0.003, local.lng), 15.0);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      _buildHeader(localesState.loading),
      Expanded(
        flex: 55,
        child: Stack(children: [
          _buildMapa(localesState.locales),
          Positioned(top: 10, left: 10, child: _buildZoomControls()),
          if (localesState.loading)
            Container(
      ...
      Expanded(
        flex: 45,
        child: _buildListaCercanos(localesState),
      ),
    ]);
  }
        ...
        ...[0.5, 1.0, 2.0, 5.0].map((r) => GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                      color: _radio == r ? AppColors.verde : AppColors.borde),
                ),
                child: Text(
                    r < 1 ? '${(r * 1000).toInt()}m' : '${r.toInt()}km',
                    style: TextStyle(
                      fontSize: 11,
                      color: _radio == r ? AppColors.verde : AppColors.texto2,
                      fontWeight: _radio == r
                          ? FontWeight.w700
                          : FontWeight.normal,
                    )),
  ...
  // ── MAPA ─────────────────────────────────────────────────────
  Widget _buildMapa(List<LocalModel> locales) {
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Container(
                    ...
                    child: Icon(Icons.sports_soccer,
                        color: sel ? AppColors.verde : Colors.black, size: 20),
                  ),
                ]),
              ),
            );
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min,
        ...
        Row(children: [
          ...
          Expanded(
              child: Text(local.nombre.toUpperCase(),
        ...
        if (local.precioDesde != null) ...[
          const SizedBox(height: 3),
          Text('Desde S/.${local.precioDesde!.toStringAsFixed(0)} / hora',
              style: const TextStyle(
  ...
  // ── LISTA ABAJO ───────────────────────────────────────────────
  Widget _buildListaCercanos(LocalesState state) {
    if (state.loading) {
      return const Center(
          child: CircularProgressIndicator(color: AppColors.verde));
    }
    ...
    return Column(children: [
      Container(
        ...
        child: Row(children: [
          ...
          const Spacer(),
          Text('${state.locales.length} locales',
      ...
      Container(
        ...
        child: GestureDetector(
          ...
          child: Container(
            ...
            child: const Center(
                child: Text('O BUSCA EN TODAS LAS CANCHAS',
                  style: TextStyle(
                    fontSize: 11, color: AppColors.texto2,
