...
import 'package:pichangaya/features/cliente/providers/reservas_cliente_provider.dart';
import 'package:pichangaya/shared/models/reserva_model.dart';

class MisReservasTab extends ConsumerStatefulWidget {
  const MisReservasTab({super.key});
  @override
  ConsumerState<MisReservasTab> createState() => _MisReservasTabState();
}

class _MisReservasTabState extends ConsumerState<MisReservasTab> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() =>
        ref.read(reservasClienteProvider.notifier).cargar());
  }
  ...
  int _totalPaginas(int total) => (total / _porPagina).ceil();

  List<ReservaModel> _reservasDePagina(List<ReservaModel> reservas, int pagina) {
    final inicio = pagina * _porPagina;
    final fin = (inicio + _porPagina).clamp(0, reservas.length);
    return reservas.sublist(inicio, fin);
  }
  ...
  Future<void> _cancelarReserva(String id) async {
    ...
    final ok = await ref.read(reservasClienteProvider.notifier).cancelar(id);
    if (!ok && mounted) {
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(reservasClienteProvider);
      ...
      Container(
        ...
        child: Row(children: [
          ...
          if (state.reservas.isNotEmpty) ...[
            Text('Pág ${_paginaActual + 1} / ${_totalPaginas(state.reservas.length)}',
                style: const TextStyle(fontSize: 12, color: AppColors.texto2)),
            const SizedBox(width: 4),
            Text('(${state.reservas.length} reservas)',
                style: const TextStyle(fontSize: 11, color: AppColors.texto2)),
          ],
          const SizedBox(width: 12),
          GestureDetector(
              onTap: () => ref.read(reservasClienteProvider.notifier).cargar(),
            ...
            : state.error != null
                ? Center(
                    child: Column(
                        ...
                        ElevatedButton(
                            onPressed: () => ref
                                .read(reservasClienteProvider.notifier)
                                .cargar(),
                    ...
                    : Column(children: [
                        Expanded(
                          child: PageView.builder(
                            ...
                            itemCount: _totalPaginas(state.reservas.length),
                            onPageChanged: (i) =>
                                setState(() => _paginaActual = i),
                            itemBuilder: (_, pagina) => _buildPagina(
                                _reservasDePagina(state.reservas, pagina)),
                          ),
                        ),
                        if (_totalPaginas(state.reservas.length) > 1)
                          _buildIndicadores(state.reservas.length),
                      ]),
      ),
    ]);
  }
  ...
  Widget _buildPagina(List<ReservaModel> reservas) {
    return RefreshIndicator(
      onRefresh: () => ref.read(reservasClienteProvider.notifier).cargar(),
  ...
  Widget _buildIndicadores(int totalReservas) {
    final total = _totalPaginas(totalReservas);
    return Container(
