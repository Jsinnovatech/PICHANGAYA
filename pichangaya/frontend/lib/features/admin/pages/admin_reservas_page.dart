...
import 'package:pichangaya/features/admin/providers/reservas_provider.dart';

class AdminReservasPage extends ConsumerStatefulWidget {
  const AdminReservasPage({super.key});
  @override
  ConsumerState<AdminReservasPage> createState() => _State();
}

class _State extends ConsumerState<AdminReservasPage> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() =>
        ref.read(adminReservasProvider.notifier).cargar());
  }

  Future<void> _cambiarEstado(String reservaId, String accion) async {
    final ok = await ref
        .read(adminReservasProvider.notifier)
        .cambiarEstado(reservaId, accion);
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(adminReservasProvider);
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const Spacer(),
            GestureDetector(
              onTap: () => ref.read(adminReservasProvider.notifier).cargar(),
                            ...
                            child: Text(f.$2,
                                style: TextStyle(
                                  ...
                                  color: state.filtroEstado == f.$1
                                      ? AppColors.verde
                                      : AppColors.texto2,
                                  fontWeight: state.filtroEstado == f.$1
                                      ? FontWeight.w700
                                      : FontWeight.normal,
                                )),
                          ),
  ...
  Widget _badgeEstado(String estado) => Container(
        ...
        child: Text(_labelEstado(estado),
            style: TextStyle(
              fontSize: 10, color: _colorEstado(estado),
...
class _DetalleReservaSheet extends StatelessWidget {
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Container(
          ...
          child: Column(children: [
            ...
            _fila('Método',   r['metodo_pago']?.toString().toUpperCase() ?? '—'),
        ...
        if (r['voucher_url'] != null) ...[
          ...
          ClipRRect(
            ...
            child: Image.network(r['voucher_url'],
  ...
  Widget _badge(String estado) {
    ...
    String label;
    switch (estado) {
      case 'confirmed': color = AppColors.verde;    label = '✅ Confirmada'; break;
      case 'pending':   color = AppColors.amarillo; label = '⏳ Pendiente';  break;
      case 'active':    color = AppColors.azul;     label = '🟢 En juego';   break;
      case 'done':      color = AppColors.texto2;   label = '🏁 Finalizada'; break;
      case 'canceled':  color = AppColors.rojo;     label = '❌ Cancelada';  break;
      default:          color = AppColors.texto2;   label = estado;
    }
    return Container(
