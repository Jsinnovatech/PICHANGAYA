...
class _State extends State<AdminShell> {
  ...
  // Nombre del admin logueado — se carga al iniciar
  ...
  Future<void> _cargarNombreAdmin() async {
    // Leer nombre del admin desde el storage
    try {
      final res = await ApiClient().dio.get('/auth/me');
      if (mounted) {
        setState(() {
          _nombreAdmin = res.data['nombre'] ?? 'Admin';
        });
      }
    } catch (_) {
      // Si falla, dejamos 'Admin' por defecto
    }
  }
  ...
  Widget _buildAdminTopBar() {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        const Spacer(),
        // ✅ Solo muestra el nombre del admin — sin botón "Ver cliente"
        Container(
