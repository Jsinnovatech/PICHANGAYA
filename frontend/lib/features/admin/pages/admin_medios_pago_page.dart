...
class _State extends State<AdminMediosPagoPage> {
  ...
  String? _qrBase64;       // base64 del QR actual (desde backend o recién elegida)
  Uint8List? _qrBytes;     // bytes de la imagen elegida localmente
  bool _loading = true;
  bool _saving  = false;
  String? _error;
  String? _success;
  ...
  Future<void> _guardar() async {
    ...
    try {
      ...
      await ApiClient().dio.put('/admin/medios-pago', data: body);
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const Text(
          'Esta información se mostrará a los clientes al realizar una reserva.',
  ...
  Widget _qrSection() {
    // Resolver bytes para mostrar imagen
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        if (previewBytes != null) ...[
          ...
          const Center(
            child: Icon(Icons.qr_code_2, color: AppColors.texto2, size: 64),
          ),
          const SizedBox(height: 8),
          const Center(
            child: Text('Sin imagen QR', style: TextStyle(color: AppColors.texto2, fontSize: 13)),
          ),
        ],
        const SizedBox(height: 12),
        SizedBox(
